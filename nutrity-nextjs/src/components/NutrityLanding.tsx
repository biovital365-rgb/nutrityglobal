...
export function NutrityLanding({ user, onStart, onAuthClick }: NutrityLandingProps) {
    ...
    useEffect(() => {
        ...
        if (!ref && typeof window !== "undefined") {
            ref = localStorage.getItem("invitation_org_id");
        }
            ...
            <main>
                    ...
                    <div className="max-w-5xl mx-auto px-6 text-center">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                            {[
                                ...
                                <div key={i} className="p-4 md:p-6 rounded-2xl border border-[var(--landing-accent)]/20 bg-[var(--landing-bg)] hover:shadow-md transition-shadow text-center">
                                    <item.icon className="w-6 h-6 md:w-8 md:h-8 text-[var(--landing-accent)] mx-auto mb-3" />
                                    <h4 className="font-serif font-bold text-[var(--landing-primary)] text-sm md:text-lg mb-1">{item.title}</h4>
                ...
                <section id="movimiento" className="py-24 bg-[var(--landing-bg)] relative border-t border-[var(--landing-accent)]/20">
                    <div className="max-w-5xl mx-auto px-6">
                        <div className="text-center mb-12">
                            <div className="inline-flex items-center gap-3 mb-6 justify-center">
                                <span className="text-3xl font-serif text-[var(--landing-accent)]">02</span>
                                <div className="h-10 w-[2px] bg-[var(--landing-accent)]"></div>
                                ...
                                {[
                                    ...
                                    <div key={i} className="flex items-center gap-4 bg-white p-5 rounded-2xl border border-[var(--landing-accent)]/10 shadow-sm">
                                        <div className="w-12 h-12 bg-[var(--landing-accent)]/10 rounded-full flex items-center justify-center shrink-0">
                                            <item.icon className="w-6 h-6 text-[var(--landing-accent)]" />
                                        </div>
                                        <div>
                                            <h4 className="font-serif font-bold text-lg text-[var(--landing-primary)]">{item.title}</h4>
                                            <p className="text-xs text-[#2d3748]">{item.desc}</p>
                                        </div>
                                    </div>
                                ))}
                ...
                <section id="estrategias" className="py-24 bg-white relative border-t border-[var(--landing-accent)]/20">
                    <div className="max-w-5xl mx-auto px-6 text-center">
                        ...
                        <button
                            onClick={onStart}
                            className="bg-[var(--landing-secondary)] text-[var(--landing-bg)] px-10 py-4 rounded-xl font-bold text-xs uppercase tracking-widest shadow-xl shadow-[var(--landing-secondary)]/20 hover:scale-105 transition-all inline-flex items-center gap-3"
                        >
                ...
                {/* Recent Blog Content */}
                <section className="py-24 bg-[var(--landing-primary)] text-[var(--landing-bg)] relative border-t-4 border-[var(--landing-accent)]">
                                ...
                                <Link href={`/blog/${post.slug}`} key={post.id} className="group flex flex-col bg-[var(--landing-bg)] rounded-[2rem] overflow-hidden hover:scale-[1.02] transition-transform duration-300">
                                    {post.thumbnail ? (
                                        ...
                                        <div className="h-48 bg-[var(--landing-accent)]/20 flex items-center justify-center">
                                            <Leaf className="w-12 h-12 text-[var(--landing-accent)]" />
                                        </div>
                                    )}
                                    <div className="p-8 flex flex-col flex-1">
                                        ...
                                        <div className="mt-auto flex items-center justify-between border-t border-[var(--landing-accent)]/20 pt-4">
                                            ...
                                            <ArrowRight className="w-4 h-4 text-[var(--landing-accent)] group-hover:translate-x-1 transition-transform" />
                                        </div>
                                    </div>
                                </Link>
                            )) : (
