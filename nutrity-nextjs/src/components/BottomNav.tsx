                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            disabled={isDisabled}
                            className={`relative flex flex-col items-center gap-1.5 p-3 rounded-2xl transition-all duration-300 ${isActive ? 'text-teal' : 'text-slate-500'
                        ...
                        >
                            <div className={`relative ${isActive ? 'scale-110' : 'scale-100'} transition-transform duration-300`}>
                                {isActive && (
                                    <motion.div
                                        layoutId="nav-glow"
                                        className="absolute inset-[-12px] bg-teal/20 blur-xl rounded-full"
                                    />
                                )}
                                <Icon className={`w-6 h-6 ${isActive ? 'drop-shadow-[0_0_8px_rgba(20,184,166,0.5)]' : ''}`} />
                            </div>
                            ...
                            {isActive && (
                                <motion.div
                                    ...
                                    className="absolute -bottom-1 w-1 h-1 bg-teal rounded-full shadow-[0_0_8px_#0d9488]"
                                />
                            )}
                        </button>
                    );
