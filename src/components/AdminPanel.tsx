...
export function AdminPanel({ user }: AdminPanelProps) {
    ...
    // ─── Menu Approval States ───────────────────────────────────────────
    const [menuUsers, setMenuUsers] = useState<any[]>([]);
                    ...
                    <motion.div key="menu-approval" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                        ...
                        <div className="grid lg:grid-cols-3 gap-6">
                            <div className="space-y-3">
                                ...
                                {users.filter(u => !u.deletedAt && u.role !== 'ADMIN').map(u => (
                                    <button
                                        key={u.id}
                                        onClick={async () => {
                                        ...
                                        <div className="flex flex-wrap items-center justify-between gap-3 p-5 bg-white rounded-2xl border border-nutrity-border shadow-sm">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    <select
                                                        value={menuPhase}
                                                        onChange={e => setMenuPhase(e.target.value as any)}
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                ...
                                                                await dbService.approveWeeklyMenu(selectedMenuUser.id, weekStart, user?.email || 'admin', menuNotes);
                                                            ...
                                                            } catch(e) { notify('error', 'Error al aprobar.'); }
                                                ...
                                                {menuWeekDays.length > 0 && menuWeekDays[0]?.status === 'PENDING' && (
                                                    <button
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                const weekStart = menuWeekDays[0]?.weekStart;
                                                                await dbService.rejectWeeklyMenu(selectedMenuUser.id, weekStart, user?.email || 'admin', reason);
                                                                setMenuWeekDays([]);
                                                                notify('success', 'Menú rechazado.');
                                                            } catch(e) { notify('error', 'Error al rechazar.'); }
                                                        }}
                                                        ...
                                                        {editingMenuRecord?.id === record.id ? (
                                                            ...
                                                            <div className="space-y-2">
                                                                {['breakfast','lunch','dinner','snack'].map(slot => (
                                                                    <div key={slot} className="flex gap-2 items-start">
                                                                        ...
                                                                        <textarea
                                                                            rows={2}
                                                                            value={editingMenuRecord.menuData?.[slot] || ''}
                                                                            onChange={e => setEditingMenuRecord((prev: any) => ({ ...prev, menuData: { ...prev.menuData, [slot]: e.target.value } }))}
                                                                            className="flex-1 text-xs bg-nutrity-bg border border-nutrity-border rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-nutrity-accent/10 resize-none"
                                                                        />
                                                                    </div>
                                                                ))}
                                                                <div className="flex gap-2 items-center">
                                                                    ...
                                                                    <input
                                                                        value={editingMenuRecord.metabolicGoal || ''}
                                                                        onChange={e => setEditingMenuRecord((prev: any) => ({ ...prev, metabolicGoal: e.target.value }))}
                                                                        className="flex-1 text-xs bg-nutrity-bg border border-nutrity-border rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-nutrity-accent/10"
                                                                    />
                                                                </div>
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <button
                                                                        onClick={() => setEditingMenuRecord({ ...record })}
